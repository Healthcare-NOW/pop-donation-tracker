from test.factories import CandidateFactory, AlertFactory, CampaignFactory
...
def test_state_summary_with_some_candidates(test_client):
    house_candidate_1 = CandidateFactory(
        campaigns=[CampaignFactory(office="H", office_district="02", office_state="MA")]
    )
    ...
    house_candidate_3 = CandidateFactory(
        campaigns=[CampaignFactory(office="H", office_district="03", office_state="MA")]
    )
    senate_candidate = CandidateFactory(campaigns=[CampaignFactory(office="S", office_state="MA")])
    CandidateFactory(campaigns=[CampaignFactory(office="S", office_state="NY")])
    CandidateFactory(campaigns=[CampaignFactory(office="S", election_year=2022, office_state="MA")])
