...
const CandidateAlertBadges = ({ alerts }) => {
  ...
  const mostRecent = maxBy(alerts, ({ created_on }) =>
    moment(created_on, "YYYY-MM-DD")
  );
  const badges = [];
...
const CandidateLink = ({ candidate }) => {
  ...
  return (
    <span className="candidateLink">
      <Link to={candidateSummaryUrl(id)} style={{}}>
        {candidateDisplayName({ candidate, maxLength: 30 })}
      </Link>
      ...
      {candidate.pledgeDate && (
        <Badge
          popupText={`Took the pledge on ${candidate.pledgeDate}`}
          icon={<PoppIcon />}
        />
      )}
      ...
      <Responsive minWidth={screenWidthThreshold + 1}>
        <Card.Group itemsPerRow={3}>
          ...
          <CandidateList
            ...
            party="Republican"
          />
          <CandidateList candidates={others} className="other" party="Other" />
        </Card.Group>
      </Responsive>
    </div>
  );
...
const StateSummary = () => {
  ...
  const { data, isLoading } = useReduxFetch({
    ...
    onSuccess: (dispatch, data) => {
      dispatch(
        stateSummarySlice.actions.receiveData({
          key: [year, state],
          ...data,
        })
      );
      dispatch(
        breadCrumbsSlice.actions.receiveData({
          state,
          year,
        })
      );
  ...
  return (
    <div>
      <Message size="large">
        <p>
          This page lists all of your state’s candidates for the U.S. Congress.{" "}
          <b>
            Click on any candidate’s name for a complete report on donations
            they have received from healthcare corporations and executives who
            are funding opposition to Medicare for All.
          </b>
        </p>

        <p>
          <b>Icons to the right of a candidate’s name</b> summarize the
        ...
        <List relaxed>
          <List.Item>
            <List.Icon>
              <div className="App-complianceIcon">
                <Capitol />
              </div>
            </List.Icon>
            <List.Content>
              is an incumbent, sitting Member of Congress
            </List.Content>
          </List.Item>
          <List.Item>
            <List.Icon>
            ...
            <List.Content>
              has taken the Patients Over Profits Pledge
            </List.Content>
          </List.Item>
          <List.Item>
            <List.Icon name="times circle" color="red" size="large" />
            <List.Content>
              has received donations from corporations funding opposition to
              Medicare for All
            </List.Content>
          </List.Item>
          <List.Item>
            ...
            <List.Content>
              ...
              have received donations from corporate executives opposing
              Medicare for All (click to see candidate’s page for more details)
            </List.Content>
          </List.Item>
        </List>
      </Message>
      ...
      <Segment basic>
        ...
        <Segment.Group>
          <Segment basic>
            {handleEmptyList(
              () => (
                <CandidateListByParty candidates={senate} />
              ),
              senate
            )}
          </Segment>
